...
<title>PHP error_log() Function</title>
<style>a.menu_func_error_log{font-weight:bold;}</style>
<meta charset="utf-8">
...
<h1>PHP <span class="color_h1">error_log()</span> Function</h1>
<hr>
<a href="php_ref_error.asp"><img class="navup" src="http://www.w3schools.com/images/up.gif" alt="PHP Error Reference" /> Complete PHP Error Reference</a>
<hr>

<h2>Definition and Usage</h2>

<p>The error_log() function sends an error to the server error log, a file or a 
remote destination.</p>
<p>This funtion returns TRUE on success, or FALSE on failure.</p>
...
<table class="reference notranslate">
  ...
  <tr>
    <td>error</td>
    <td>Required. The error message to log</td>
  </tr>
	<tr>
    <td>type</td>
    <td>Optional. Specifies the error log type. <br>
	Possible log types:<ul>
		<li>0 - Default. The error is sent to the servers logging system or a file, 
		depending on how the error_log configuration is set in the php.ini file</li>
		<li>1 - The error is sent by email to the address in the destination 
		parameter. This message type is the only one that uses the headers 
		parameter </li>
		<li>2 - The error is sent through the PHP debugging connection. This 
		option is only available in PHP 3</li>
		<li>3 - The error is added to the file destination string</li>
	</ul>
		</td>
  </tr>
  <tr>
    <td>destination</td>
    <td>Optional. Specifies where to send the error message. The 
	value of this parameter depends on the value of the &quot;type&quot; parameter</td>
  </tr>
  <tr>
    <td>headers</td>
    <td>Optional. Only used if the &quot;type&quot; parameter is &quot;1&quot;. 
	Specifies additional headers, like From, Cc, and Bcc. The additional headers 
	should be separated with a CRLF (\r\n). 
	<p><b>Note:</b> When sending an email, it must contain a From header. This 
	can be set with this parameter or in the php.ini file.</td>
  </tr>
  </table>
<br>
...
<h2>Example</h2>
<p>The following example sends an e-mail with a custom error:</p>
...
if ($test&gt;1)<br>
{<br>
error_log(&quot;A custom error has been triggered&quot;,<br>
1,&quot;someone@example.com&quot;,&quot;From: webmaster@example.com&quot;);<br>
}<br>
?&gt;

</div></div>

<p>The mail received from the code above looks like this:</p>
...
<div class="footer">
	...
	<div id="footerAbout">
		<a href="func_error_log.asp" onclick="displayError();return false" style="word-spacing:normal;">REPORT ERROR</a> |
		<a href="../default.asp" target="_top">HOME</a> |
		<a href="func_error_log.asp#top" target="_top">TOP</a> |
		<a href="func_error_log.asp" target="_blank" onclick="printPage();return false">PRINT</a> |
